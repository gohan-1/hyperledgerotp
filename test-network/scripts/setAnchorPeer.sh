...
createAnchorPeerUpdate() {
  ...
  if [ $ORG -eq 1 ]; then
    HOST="peer0.org1.valtrans.com"
    PORT=7051
  elif [ $ORG -eq 2 ]; then
    HOST="peer0.org2.valtrans.com"
    PORT=9051
  elif [ $ORG -eq 3 ]; then
    HOST="peer0.org3.valtrans.com"
    PORT=11051
  else
...
updateAnchorPeer() {
  peer channel update -o localhost:7050 --ordererTLSHostnameOverride orderer.valtrans.com -c $CHANNEL_NAME -f ${TEST_NETWORK_HOME}/channel-artifacts/${CORE_PEER_LOCALMSPID}anchors.tx --tls --cafile "$ORDERER_CA" >&log.txt
  res=$?
  cat log.txt
